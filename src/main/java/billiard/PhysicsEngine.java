...
public abstract class PhysicsEngine implements SubEngine {
    ...
    public PhysicsEngine(LCPSolverConfiguration solverConfig, double simulationTimeStep) {
        ...
        this.simulationTimeStep = Math.min(Math.max(simulationTimeStep, BilliardEngine.minTimeStep), BilliardEngine.maxTimeStep);
    ...
    protected void setSimulationTimeStep(double newTimeStep) {
        simulationTimeStep = Math.min(Math.max(newTimeStep, BilliardEngine.minTimeStep), BilliardEngine.maxTimeStep);
    }

    protected void updateTimeStep() {
        if (simulationTimeStep < BilliardEngine.maxTimeStep || simulationTimeStep == BilliardEngine.minTimeStep) {
            simulationTimeStep = BilliardEngine.maxTimeStep;
        }
    }
